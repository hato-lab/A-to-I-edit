...
allAtoG<-allAtoG[allAtoG$Location %in% atLeast3,] #where at least 4 samples have any edits
...
temptbl[,3]<-as.numeric(temptbl[,3])
temptbl[,3]<-temptbl[,3]+1
#temptbl[,1]<-str_extract(temptbl[,1],"\\d+")
temptbl<-data.frame(temptbl,stringsAsFactors = FALSE)
temptbl<-transpose(as.list(temptbl))
all.genes<-NULL
for(i in 1:length(temptbl)){
...
#olddf<-read.delim("/N/project/jeredSlate/2022/GY_AtoI/allgenes_everyLocation_byStrand.tsv",row.names = TRUE)
...
#matchingLocations<-allsites_info$location[which(allsites_info$location %in% olddf$V1)]
...
#allsites_info[,c(6:9)]<-ttttemp[,6:9]
...
replengthDF[,1]<-gsub("ZZZ","?",replengthDF[,1]) #an easy to remove separator
...
c(paste0("HRT0",seq(1,9,1)),paste0("HRT",seq(10,14,1)))) ###DKD02 and 06 are removed
...
##Find which edit sites are significant between conditions
  ...
  #condition<-c("GY1_C_S118","GY1_N_S119","GY2_C_S120","GY2_N_S121","GY3_C_S122","GY3_N_S123","GY4_C_S124","GY4_N_S125","GY5_C_S126","GY5_N_S127","GY6_C_S128","GY6_N_S129","GY7_C_S130","GY7_N_S131","GY8_C_S132","GY8_N_S133","GY9_C_S134","GY9_N_S135","GY10_C_S136","GY10_N_S137","GY11_C_S138","GY11_N_S139","GY12_C_S140","GY12_N_S141","GY13_C_S142","GY13_N_S143","GY14_C_S144","GY14_N_S145","GY15_C_S146","GY15_N_S147")
...
,ylab = "Density",xlab="%Edit",xlim=c(0,1),main = paste0("Human samples % Edited in Site By Condition"),col=alpha(colorsused[1], 1),ylim=c(0,13)))
  #print(lines(density(rowMeans(azinSites[,grep("CKD",colnames(azinSites))])),col=alpha(colorsused[2], 1)))
...
#box_plot <- ggplot(data_air_nona, aes(x = Month, y = Ozone))+geom_boxplot()
...
#intronPre[,3]<-gsub("\\|chr.+","",intronPre[,3])
intronPre$Start<-NA
intronPre$End<-NA
...
for(edit in 1:length(editSiteList)){
	...
	if(editSite[3]=="+"){
		...
		#10 bins (smalldf[region,2]+5):(smalldf[region,3]-40)
...
#smalldf$start<-(smalldf$start+1)
#smalldf$end<-(smalldf$end-1)
...
#23bins, 3nt, 20 chunks, 30nt, 10nt
#editSitesSame
#smalldf
